...
use watson_discovery_api::Api;
use watson_discovery_api::GetEnvironmentResponse::{BadRequest,
                                                   EnvironmentFetched};
...
pub fn show_environment(creds: Credentials, matches: &clap::ArgMatches) {
    ...
    match info.client
                .get_environment(env_id, VERSION.to_string())
                .wait() {
        Ok(response) => {
            match response {
                EnvironmentFetched(success) => {
                    println!(
                        "{}",
                        to_string_pretty(&success).expect(
                            "Internal error: failed to format Environment",
                        )
                    )
                }
                BadRequest(failure) => println!("Bad request {:?}", failure),
            }
        }
